...
export function CRMLeads() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let supabaseError = "Erro desconhecido";
      if (error instanceof Error) {
        supabaseError = error.message;
      } else if (typeof error === "object" && error !== null) {
        const errorObj = error as Record<string, unknown>;
        supabaseError = (errorObj.message as string) || 
      ...
      toast({
        ...
        description: supabaseError || "Erro desconhecido. Veja o console.",
        variant: "destructive",
      });
  ...
  const avgProbability =
    filteredLeads.length > 0
      ? Math.round(
          filteredLeads.reduce((sum, lead) => sum + lead.probability, 0) /
            filteredLeads.length,
        )
      : 0;
  const activeLeads = filteredLeads.filter(
    (lead) => lead.status !== "perdido" && lead.status !== "fechamento",
  ).length;
  const conversionRate =
    leads.length > 0
      ? Math.round(
          (filteredLeads.filter((l) => l.status === "fechamento").length /
            leads.length) *
            100,
        )
      : 0;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("next_follow_up", e.target.value)
                    }
                  />
                </div>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      handleEditInputChange("source", value)
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleEditInputChange("next_follow_up", e.target.value)
                    }
                  />
                </div>
              ...
              <div className="flex justify-end space-x-2 pt-4">
                ...
                <Button
                  onClick={() =>
                    handleEditSave(leads.find((l) => l.id === editingId)!)
                  }
                  variant="fluxo"
                  className="bg-gradient-fluxo"
