...
export function CRMDashboard() {
  ...
  const avgProbability = leads.length > 0 ? Math.round(leads.reduce((sum, lead) => sum + lead.probability, 0) / leads.length) : 0;
  const closedLeads = leads.filter(l => l.status === 'fechamento').length;
  const lostLeads = leads.filter(l => l.status === 'perdido').length;
  const activeLeads = leads.filter(l => l.status !== 'perdido' && l.status !== 'fechamento').length;
  const conversionRate = totalLeads > 0 ? Math.round((closedLeads / totalLeads) * 100) : 0;
  const winRate = (closedLeads + lostLeads) > 0 ? Math.round((closedLeads / (closedLeads + lostLeads)) * 100) : 0;
  ...
  // Leads por status
  const statusCounts = Object.keys(statusLabels).map(status => leads.filter(l => l.status === status).length);
  const statusValues = Object.keys(statusLabels).map(status => leads.filter(l => l.status === status).reduce((sum, l) => sum + l.value, 0));
  ...
  const funnelData = {
    ...
    datasets: [
      ...
      },
      {
        label: 'Valor Total (R$)',
        data: statusValues,
        backgroundColor: Object.keys(statusLabels).map(s => statusColors[s] + '99'),
        borderRadius: 8,
        borderSkipped: false,
        yAxisID: 'y1',
      }
    ...
    datasets: [
      {
        ...
        backgroundColor: Object.keys(sourceCounts).map(source => sourceColors[source as keyof typeof sourceColors] || '#6b7280'),
        borderWidth: 2,
        borderColor: '#ffffff',
      }
    ]
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="bg-gradient-to-br from-green-50 to-green-100 border-green-200">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-green-700">Valor Potencial</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="text-xs text-green-600 mt-1">
              Média: R$ {totalLeads > 0 ? (totalValue / totalLeads).toLocaleString('pt-BR', { maximumFractionDigits: 0 }) : '0'}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-purple-800">{conversionRate}%</div>
        ...
        <Card className="bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-orange-700">Probabilidade Média</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-orange-800">{avgProbability}%</div>
                ...
                <div key={lead.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                  ...
                  <div className="text-right">
                    <div className="font-medium text-sm">
                      {lead.next_follow_up ? new Date(lead.next_follow_up).toLocaleDateString('pt-BR') : 'Não agendado'}
                    </div>
                    <div className="text-xs text-gray-500">
                      R$ {lead.value.toLocaleString('pt-BR')} • {lead.probability}%
                    </div>
                  </div>
                </div>
              ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {Object.entries(statusLabels).map(([status, label]) => {
          ...
          const avgProb = statusLeads.length > 0 ? Math.round(statusLeads.reduce((sum, l) => sum + l.probability, 0) / statusLeads.length) : 0;
            ...
            <Card key={status} className="hover:shadow-md transition-shadow">
              ...
              <CardContent className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">R$ {statusValue.toLocaleString('pt-BR')}</span>
                </div>
                <div className="flex justify-between text-sm">
