...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      "Configurações encontradas - Assistant ID:",
      !!assistantId,
      "API Key:",
      !!openAIApiKey,
    );
    ...
    if (!currentThreadId) {
      ...
      const threadResponse = await makeOpenAIRequest(
    ...
    const runResponse = await makeOpenAIRequest(
    ...
    const messagesResponse = await makeOpenAIRequest(
